...
\section{Prototype document functions}
...
\pkg{template} also makes it easier for \LaTeX{} programmers to provide
...
\subsection {Objects, templates, and instances}
...
\begin{decl}
  |\DeclareObjectType| \arg{obj.~type} \arg{no.~args}
\end{decl}
...
\begin{decl}
  |\DeclareTemplateInterface| \arg{obj.~type} \arg{template} \arg{no.~args}
    \arg{keys}
\end{decl}
...
out different parts. Each \meta{key} must have a
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    \ttfamily boolean    & \texttt{true} or \texttt{false}            \\
    \ttfamily choice\arg{choices}
      & A list of pre-defined \meta{choices} \\
    \ttfamily code
      & Generalised key type: use |#1| as the input to the key \\
    \ttfamily commalist  & A comma-separated list                        \\
    \ttfamily function\arg{$N$}
      & A function definition with $N$ arguments
       ($N$ from $0$ to $9$) \\
    \ttfamily instance\arg{name}
      & An instance of type \meta{name} \\
    \ttfamily integer    & An integer or integer expression            \\
    \ttfamily length     & A fixed length                              \\
    \ttfamily muskip     & A math length with shrink and stretch components \\
    \ttfamily real       & A real (floating point) value               \\
    \ttfamily skip       & A length with shrink and stretch components \\
    \ttfamily tokenlist  & A token list: any text or commands          \\
    \bottomrule
  \end{tabular}
...
\begin{decl}
  |\DeclareTemplateCode| \arg{obj.~type} \arg{template} \arg{no.~args}
...
register. As illustrated, the key word \enquote{global}
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    \ttfamily boolean    & Boolean variable, \emph{e.g}.~\cs{l_tmpa_bool}      \\
    \ttfamily choice
      & List of choice implementations
      (see Section~\ref{sec:choices-key}) \\
    \ttfamily code
      & \meta{code} using |#1| as input to the key \\
    \ttfamily commalist  & Comma list, \emph{e.g}.~\cs{l_tmpa_clist}           \\
    \ttfamily function
      & Function taking $N$ arguments, \emph{e.g}.~\cs{use_i:nn}     \\
    \ttfamily instance \\
    \ttfamily integer    & Integer variable, \emph{e.g}.~\cs{l_tmpa_int}       \\
    \ttfamily length     & Dimension variable, \emph{e.g}.~\cs{l_tmpa_dim}     \\
    \ttfamily muskip     & Muskip variable, \emph{e.g}.~\cs{l_tmpa_muskip}     \\
    \ttfamily real       & Floating-point variable, \emph{e.g}.~\cs{l_tmpa_fp} \\
    \ttfamily skip       & Skip variable, \emph{e.g}.~\cs{l_tmpa_skip}         \\
    \ttfamily tokenlist  & Token list variable, \emph{e.g}.~\cs{l_tmpa_tl}     \\
    \bottomrule
  \end{tabular}
...
\begin{decl}
  |\DeclareInstance| \arg{obj.~type} \arg{instance} \arg{template}
    \arg{parameters}
\end{decl}
...
\begin{decl}
  |\UseInstance| \arg{obj.~type} \arg{instance} \arg{parameters}
    \meta{arguments}
\end{decl}
...
\begin{decl}
  |\UseTemplate| \arg{obj.~type} \arg{template} \arg{settings}
    \meta{arguments}
\end{decl}
